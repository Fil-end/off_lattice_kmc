...
class RunKMC():
    def __init__(self,
        ...
        self.calculator = Calculator(
            model_path=model_path, calculate_method=calculate_method, temperature_K=TEMPERATURE_K)
        self.atoms, initial_energy, _ = self.calculator.to_calc(
            self.initial_slab, calc_type='opt')
        ...
        self.initial_energy = initial_energy + self.env.n_O2 * \
            self.env.E_O2 + self.env.n_O3 * self.env.E_O3
        self.max_observation_atoms = max_observation_atoms
        self.pd = nn.ZeroPad2d(
            padding=(0, 0, 0, self.max_observation_atoms-len(self.atoms.get_positions())))
        ...
        results = ['energies', 'actions', 'structures',
                   'timesteps', 'barriers', 'adsorbates', 'symbols']
        self.history = {}
        for item in results:
            self.history[item] = []
        self.history['energies'] = [self.initial_energy]
        self.history['actions'] = [0]
        self.history['structures'] = [
    ...
    def run(self, atoms: ase.Atoms, num_episode: int) -> ase.Atoms:
        ...
        atoms, tmp_state_dict = self.env.step(
            atoms, previous_energy, num_episode)
        ...
        if prob > np.random.random():
            ...
            barrier = 10E-8
            print(self.history['adsorbates'][-1])
        ...
        self.pd = nn.ZeroPad2d(
            padding=(0, 0, 0, self.max_observation_atoms-len(atoms.get_positions())))
        self.update_history(
            atoms, tmp_state_dict['action'], energy, barrier, adsorbates)
    ...
    def update_history(self, atoms: ase.Atoms, action: int, energy: float, barrier: float, adsorbates: Tuple[int, int]) -> None:
        self.history['structures'].append(
            np.array(self.pd(torch.tensor(atoms.get_positions())).flatten()))
        self.history['energies'].append(energy)
        self.history['actions'].append(action)
        self.history['timesteps'].append(self.history['timesteps'][-1] + 1)
        self.history['real_time'].append(
            self.history['real_time'][-1] + KCAL_2_EV * barrier / (R * TEMPERATURE_K))
    ...
    def _preprocessing(self, system: ase.Atoms) -> ase.Atoms:
        zeolite = system.copy()
        del zeolite[[a.index for a in zeolite if a.symbol ==
                     self.env.cluster_metal]]
        initial_cluster = system.copy()
        del initial_cluster[[
            a.index for a in initial_cluster if a.symbol != self.env.cluster_metal]]
    ...
    def iteration(self, atoms: ase.Atoms, log_dir: str, max_episodes: int, d: int,
        ...
        if load_pkl:
            atoms = self._generate_initial_slab()
            data = np.load(
                './save_dir/history/kmc_info.npz', allow_pickle=True)
            self.env.n_O2, self.env.n_O3 = data['adsorbates']
            self.history = {}
            for key in data:
                self.history[key] = data[key].tolist()

            current_atoms = np.array(self.history['structures'])[-1].\
                            reshape(int(len(self.history['structures'][-1])/3),3)
            current_symbols =  self.history['symbols'][-1]
            
            for atom_idx in range(len(current_atoms)):
                positions = current_atoms[atom_idx]
                if (positions[0] != 0.000 or positions[1] != 0.000 or positions[2] != 0.000) \
                    and current_symbols[atom_idx] != self.env.cluster_metal:
                    ele = Atom(current_symbols[atom_idx], positions)
                    atoms = atoms + ele
            print(atoms)
...
if __name__ == '__main__':
    model = RunKMC(calculate_method='MACE', model_path='PdSiOH.model', save_dir='save_dir',
                   max_observation_atoms=500, save_every=100, in_zeolite=False)
    atoms = model.initial_slab

    log_dir = './save_dir/save_model/'
    model.iteration(atoms, log_dir=log_dir, max_episodes=int(
        10E6), d=int(10E3), load_pkl=False)
    # atoms = read_arc('chk_pt.arc')[0]
    # data = np.load('./save_dir/history/kmc_info.npz', allow_pickle=True)
